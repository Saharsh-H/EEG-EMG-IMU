...
for trial_no in range(1, num_trials + 1):
    df = pd.read_csv(base_dir / f"trial_{trial_no:02d}" / "imu_wrist_filtered.csv")

    N = len(df)
    time_sec = np.linspace(0, 6, N)

    out_dir = base_dir / f"Plots/Wrist_IMU_Filtered/Trial {trial_no}"
    ...
    for channel in imu_channels:
        ...
        ax.set_title(f"Trial {trial_no} Filtered Wrist IMU {channel} Data")
